...
namespace ChessPrototype.Unity.UI
{
    public sealed class PrototypeUIController : MonoBehaviour
    {
        ...
        public void ShowPiece(UnitRuntime unit)
        {
            ...
            if (pieceStatusText != null) pieceStatusText.text = BuildStatusText(unit);
        ...
        public void RefreshMap()
        {
            ...
            foreach (var kv in _map.Nodes)
            {
                ...
                if (txt != null) txt.text = $"{n.id} [{n.type}]";
                b.interactable = n.available && !n.completed;
                var bg = b.GetComponent<Image>();
                if (bg != null)
                {
                    bg.color = n.completed ? new Color(0.2f, 0.65f, 0.25f, 0.95f) :
                        n.available ? new Color(0.2f, 0.4f, 0.9f, 0.95f) :
                        new Color(0.15f, 0.15f, 0.2f, 0.7f);
                }
                var rt = b.transform as RectTransform;
                var p = new Vector2(28 + n.lane * 110, -24 - n.tier * 84);
                if (rt != null)
                {
                    ...
                    rt.sizeDelta = new Vector2(102, 28);
                }
                var captured = n.id;
                b.onClick.AddListener(() =>
                {
                    ...
                    if (selected.type == MapNodeType.Battle || selected.type == MapNodeType.Elite || selected.type == MapNodeType.Boss) ShowBattle();
            ...
            foreach (var kv in _map.Edges)
            {
                ...
                for (var i = 0; i < toList.Count; i++)
                {
                    ...
                    CreateMapLine(fromPos + new Vector2(50, -14), toPos + new Vector2(50, -14), highlighted);
                }
            }
        }
        ...
        private void CreateMapLine(Vector2 from, Vector2 to, bool highlighted)
        {
            ...
            img.color = highlighted ? new Color(0.85f, 0.45f, 0.15f, 0.9f) : new Color(1f, 1f, 1f, 0.2f);
        ...
        private void RefreshHud()
        {
            ...
            if (intentLines != null) intentLines.RenderPlan(plan);
            RefreshCardButtonSelection();
        }
        ...
        private static string BuildStatusText(UnitRuntime unit)
        {
